...
class Test(testlib.TestCase):
    ...
    def test_secret_example3(self):
        matches  = [ "aac","ccc","caa" ]
        k        = 2
        expected = [1, 0, 2]
        return self.do_test(matches, k, expected)

    def test_secret_example3bis(self):
        matches  = [ "ccc", "caa", "aac"]
        k        = 2
        expected = [0, 2, 1]
        return self.do_test(matches, k, expected)

    def test_secret_example4(self):
        matches  = [ "aac","ccc","caa" ]
        k        = 1
        expected = [0, 2, 1]
        return self.do_test(matches, k, expected)

    def test_secret_example4bis(self):
        matches  = [ "ccc","caa","aac" ]
        k        = 1
        expected = [2, 1, 0]
        return self.do_test(matches, k, expected)
    ...
    def test_secret_runic_tie(self, matches, k, expected):
          return self.do_test(matches, k, expected)


    @file_data("test_emojismall_tie.json")
    def test_secret_emojismall_tie(self, matches, k, expected):
          return self.do_test(matches, k, expected)


    @file_data("test_emojismall_evil.json")
    def test_secret_emojismall_evil(self, matches, k, expected):
          return self.do_test(matches, k, expected)


    @file_data("test_joyokanji_random_tie.json")
    def test_secret_joyokanji_random_tie(self, matches, k, expected):
          return self.do_test(matches, k, expected)


    @file_data("test_hiragana_random_tie.json")
    def test_secret_hiragana_random_tie(self, matches, k, expected):
          return self.do_test(matches, k, expected)


    @file_data("test_emoji_random_tie.json")
    def test_secret_emoji_random_tie(self, matches, k, expected):
          return self.do_test(matches, k, expected)
